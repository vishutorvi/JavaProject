...
public class PhaseTwoCode {
	...
	public int matchingEntityCount(Map<String, ArrayList<String>> finalObjectStructure){
		...
		int userInnerEntities = 7;
		ArrayList<String> matchingEntities;
		for(Map.Entry<String, ArrayList<String>> matchingObject:finalObjectStructure.entrySet()){
			if(matchingObject.getKey().equals("users")){
				matchingEntities = matchingObject.getValue();
				for(String userObject: matchingEntities){
					if(userInnerEntities >= 1){
						countMatchingEntity = userInnerEntities + countMatchingEntity;
						userInnerEntities = userInnerEntities - 1;
					}
				}
			}
			if(matchingObject.getKey().equals("repos")){
				matchingEntities = matchingObject.getValue();
				if(matchingEntities.size() == 2){
					countMatchingEntity = countMatchingEntity + 2;
				}
			}
		}
